...
describe('Admin API Integration Tests', () => {
  ...
  describe('Metrics Endpoints (Authenticated)', () => {
    ...
    it('should get metrics overview', async () => {
      ...
      console.log(`📊 Metrics Overview: ${response.body.totalUniqueUsers} users, ${response.body.messagesCount} messages, DAU: ${response.body.dau}`);
    });
    ...
    it('should get new users metrics', async () => {
      ...
      console.log(`👥 New Users Data: ${response.body.data.length} data points`);
    });

    it('should get active users metrics', async () => {
      if (!dbAvailable) return; // Skip if database not available

      const response = await authorizedRequest.get('/api/metrics/active?period=7d');
  ...
  describe('Users Endpoints (Authenticated)', () => {
    ...
    it('should get users list', async () => {
      ...
      console.log(`👥 Users List: ${response.body.items.length} users returned, total: ${response.body.total}`);
    });
  });
