...
export class DatabaseTestUtils {
  static async ensureDatabaseConnection(): Promise<boolean> {
    try {
      ...
      console.warn('[test] Database connection failed:', error instanceof Error ? error.message : String(error));
...
export class TestUtils {
  static async authenticateTelegram(): Promise<AuthToken> {
    ...
    if (response.status !== 200) {
      throw new Error(`Debug authentication failed: ${response.status} - ${response.text}`);
    }
