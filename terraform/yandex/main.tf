...
resource "yandex_storage_bucket" "video-translate-vtrans-service-env" {
...
# =============================================================================
# Video Translation Service (Microservice)
# =============================================================================

resource "yandex_function" "vtrans-service" {
  name               = "vtrans-service"
  user_hash          = filebase64sha256("packages/vtrans-service/vtrans-service.zip")
  runtime            = "nodejs18"
  ...
  package {
    bucket_name = yandex_storage_bucket.video-translate-bot-code.id
    object_name = "vtrans-service.zip"
  }
...
resource "yandex_api_gateway" "vtrans-service-gateway" {
  name        = "vtrans-service-gateway"
  description = "API Gateway for Video Translation Service"
...
paths:
  /translate:
    post:
      x-yc-apigateway-integration:
        type: cloud_functions
        function_id: "${yandex_function.vtrans-service.id}"
        tag: "$latest"
        service_account_id: "${var.service_account_id}"
      responses:
        "200":
...
output "vtrans_service_gateway_domain" {
  value = yandex_api_gateway.vtrans-service-gateway.domain
}
